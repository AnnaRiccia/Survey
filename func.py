...
def plot_graph_for_column1(df, column_name):
    """
    Mappa le risposte testuali ai valori numerici e genera un grafico a torta e a barre per una colonna specifica del dataframe.
    """
    response_map = {
        "Molto D'accordo": 4,
        "D'accordo": 3,
    ...
    df[column_name] = df[column_name].map(response_map)
    ...
    fig.add_trace(
        go.Bar(
            ...
            textposition='auto',  # Testo sopra le barre
            marker=dict(
                color=['#ffcc66', '#ff9933', '#ff6600', '#cc3300', '#990000'][:len(competency_counts)]
            )
        ),
        row=1, col=2
    )
    
    title = titles_dict.get(column_name, "Titolo Generico")
...
def plot_cdh_conoscenze(df):
    fig = plot_graph_for_column1(df, 'cdh_conoscenze')
    st.plotly_chart(fig, use_container_width=True, key='cdh_conoscenze')

def plot_cdh_competenze_tecniche(df):
    fig = plot_graph_for_column1(df, 'cdh_competenze_tecniche')
    st.plotly_chart(fig, use_container_width=True, key='cdh_competenze_tecniche')

def plot_cdh_abilita_analitiche(df):
    fig = plot_graph_for_column1(df, 'cdh_abilita_analitiche')
    st.plotly_chart(fig, use_container_width=True, key='cdh_abilita_analitiche')

def plot_cdh_innovazione(df):
    fig = plot_graph_for_column1(df, 'cdh_innovazione')
    st.plotly_chart(fig, use_container_width=True, key='cdh_innovazione')

def plot_cdh_formazione(df):
    fig = plot_graph_for_column1(df, 'cdh_formazione')
    st.plotly_chart(fig, use_container_width=True, key='cdh_formazione')
...
# Dizionario che mappa le risposte a valori numerici
mappa_risposte = {
    'Molto D\'accordo': 4,
    'D\'accordo': 3,
    'Neutrale': 2,
...
# Funzione generica per creare un grafico
def plot_graph(df, column_name):
    # Conta la distribuzione delle risposte nella colonna
    df[column_name] = df[column_name].map(mappa_risposte)
    competency_counts = df[column_name].value_counts()
    ...
    mappa_risposte_inversa = {
        ...
        2: 'Neutrale',
        1: 'In disaccordo',
        0: 'Nessuna risposta'  # Per i NaN, possiamo usare "Nessuna risposta"
    }
    ...
    fig.add_trace(
        go.Pie(
            ...
            values=competency_counts.values,
            marker=dict(
                colors=['#228B22', '#8fbc8f', '#66cdaa', '#2e8b57', '#006400']  # Gradazione di verde
            ),
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=competency_counts.values,
            marker=dict(
                color=['#228B22', '#8fbc8f', '#66cdaa', '#2e8b57', '#006400'][:len(competency_counts)],  # Colori ordinati
            ),
...
# Funzioni separate per ogni grafico
def plot_hardware(df):
    fig = plot_graph(df, 'infr_hardware')  # Nome corretto della colonna
    st.plotly_chart(fig, use_container_width=True, key='hardware_button')

def plot_software(df):
    fig = plot_graph(df, 'infr_software')  # Nome corretto della colonna
    st.plotly_chart(fig, use_container_width=True, key='software_button')

def plot_cloud(df):
    fig = plot_graph(df, 'infr_cloud')  # Nome corretto della colonna
    st.plotly_chart(fig, use_container_width=True, key='cloud_button')

def plot_sicurezza(df):
    fig = plot_graph(df, 'infr_sicurezza')  # Nome corretto della colonna
    st.plotly_chart(fig, use_container_width=True, key='sicurezza_button')
...
def plot_graph_for_column2(df, column_name):
    df[column_name] = df[column_name].map(mappa_risposte)
    competency_counts = df[column_name].value_counts()
...
def plot_cdh_interazione(df):
    """
    Genera il grafico per la colonna 'eco_interazione' in formato a torta e a barre.
    """
    fig = plot_graph_for_column2(df, "eco_interazione")  # Chiama la funzione generica passando il nome della colonna
    st.plotly_chart(fig, use_container_width=True, key='eco_interazione')

# Funzione per il grafico delle piattaforme digitali
def plot_cdh_piattaforme(df):
    """
    Genera il grafico per la colonna 'eco_piattaforme' in formato a torta e a barre.
    """
    fig = plot_graph_for_column2(df, "eco_piattaforme")  # Chiama la funzione generica passando il nome della colonna
    st.plotly_chart(fig, use_container_width=True, key='eco_piattaforme')

# Funzione per il grafico dei processi digitalizzati
def plot_cdh_processi(df):
    """
    Genera il grafico per la colonna 'eco_processi' in formato a torta e a barre.
    """
    fig = plot_graph_for_column2(df, "eco_processi")  # Chiama la funzione generica passando il nome della colonna
    st.plotly_chart(fig, use_container_width=True, key='eco_processi')
