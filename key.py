...
def hist_soddisfazione_maturita(df):
    # Assicurati che la funzione 'mappa_maturita' sia stata applicata in precedenza
    mappa_maturita(df)  # Applica la mappatura sulla maturità digitale
    ...
    fig = px.histogram(
        df,  # DataFrame con le colonne 'soddisfazione' e 'maturita_digitale'
...
def mappa_risposte(df):
    ...
    # Rimuovi 'Nessuna risposta' e applica la mappatura
    df[['infr_hardware', 'infr_software', 'infr_cloud', 'infr_sicurezza']] = df[['infr_hardware', 'infr_software', 'infr_cloud', 'infr_sicurezza']].replace('Nessuna risposta', np.nan)
    ...
    for col in ['infr_hardware', 'infr_software', 'infr_cloud', 'infr_sicurezza']:
        df[col] = pd.to_numeric(df[col], errors='coerce')  # Converte in numerico, sostituendo eventuali errori con NaN

# Funzione per creare il grafico con intensità del colore basata sul conteggio delle aziende
def visualizza_maturita_infrastrutture(df):
    ...
    df = df[df['maturita_digitale'] != 'Nessuna risposta']
    ...
    conteggi = df['maturita_digitale'].value_counts().reset_index()
    conteggi.columns = ['maturita_digitale', 'conteggio']

    for infrastruttura in infrastrutture:
        media_infrastruttura = df.groupby('maturita_digitale')[infrastruttura].mean().reset_index()
...
def mappa_risposte1(df):
    ...
    # Rimuovi 'Nessuna risposta' e applica la mappatura
    df[['cdh_conoscenze', 'cdh_competenze_tecniche', 'cdh_abilita_analitiche', 'cdh_innovazione', 'cdh_formazione']] = df[['cdh_conoscenze', 'cdh_competenze_tecniche', 'cdh_abilita_analitiche', 'cdh_innovazione', 'cdh_formazione']].replace('Nessuna risposta', np.nan)
...
# Funzione per creare il grafico con intensità del colore basata sul conteggio delle aziende
def visualizza_maturita_figure(df):
    titles_dict = {
    ...
    df[['cdh_conoscenze', 'cdh_competenze_tecniche', 'cdh_abilita_analitiche', 'cdh_innovazione', 'cdh_formazione']] = df[['cdh_conoscenze', 'cdh_competenze_tecniche', 'cdh_abilita_analitiche', 'cdh_innovazione', 'cdh_formazione']].fillna(0)
    ...
    # Filtra per rimuovere "Nessuna risposta" dalla colonna 'maturita_digitale'
    df = df[df['maturita_digitale'] != 'Nessuna risposta']
    df = df[df['maturita_digitale'] != 'Non digitalizzato'] 
    ...
    for col in columns:
        media_col = df.groupby('maturita_digitale')[col].mean().reset_index()
...
def mappa_maturita(df):
    ...
    if 'maturita_digitale' in df.columns:
        df['maturita_digitale'] = df['maturita_digitale'].replace(values)
        df['maturita_digitale'].fillna('Nessuna risposta', inplace=True)

def analizza_relazione_inizio_maturita_heatmap(df):
    """
    ...
    if 'inizio_trans' not in df.columns or 'maturita_digitale' not in df.columns:
    ...
    tabella_contingenza = df.groupby(["inizio_trans", "maturita_digitale"]).size().reset_index(name="conteggio")
...
#################################################################################################### MATURITA E LEADER ####################################################################################################
def analizza_maturita_leader(df):
    """
    Crea una heatmap che mostra la relazione tra il livello di maturità digitale 
    ...
    # Verifica che le colonne esistano
    if 'coinvolgimento_leader' not in df.columns or 'maturita_digitale' not in df.columns:
    ...
    df['coinvolgimento_leader'] = pd.to_numeric(df['coinvolgimento_leader'], errors='coerce')  # Converte in numerico, NaN per valori non numerici
    df['coinvolgimento_leader'] = df['coinvolgimento_leader'].fillna(0)  # Rimpiazza NaN con 0
    df['coinvolgimento_leader'] = df['coinvolgimento_leader'].apply(lambda x: x if 0 <= x <= 5 else 0)  # Rimpiazza valori fuori dall'intervallo [0,5] con 0
    ...
    # Creiamo la tabella di contingenza
    tabella_contingenza = df.groupby(["coinvolgimento_leader", "maturita_digitale"]).size().reset_index(name="conteggio")
